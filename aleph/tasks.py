...
def process(self, sample_data, metadata, track_data={}):
    ...
    self.logger.info(
        "Recieved sample %s (%s) for processing"
        % (sample_id, metadata[FIELD_SAMPLE_FILETYPE])
    )
    ...
    if track_data:
        ...
        if "children" in track_data.keys():
            ...
            for child in children:
                if child != sample_id:
                    call_task(
                        "aleph.datastores.tasks.track",
                        args=[child, {"parents": [sample_id]}],
                    )
...
def dispatch(component_type, sample):
    ...
    for loader, plugin_name, is_pkg in plugins:
        ...
        try:
            ...
            call_task(
                "aleph.%ss.tasks.run" % component_type,
                args=[plugin_name, sample],
