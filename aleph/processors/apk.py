...
class APK(Processor):
    """
    Plugin that analyze APK files and extracts static properties.
    ...
    def parse_apk(self, data):
        ...
        try:
            ...
            if apk.is_valid_APK():
                ...
                self.logger.debug(
                    "Zip file %s is not a valid APK file" % self.sample.path
                )
        except Exception as ex:
            self.logger.warning(
                "Could not parse %s because of %s", self.sample.path, ex
            )
