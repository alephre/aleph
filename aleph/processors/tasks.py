...
def run(self, processor_name, sample):
    ...
    try:
        ...
        self.logger.error('Error while running processor %s on sample %s: %s' % (processor_name, sample['id'], e.get_exception_text()))
    except Exception as e:
        self.logger.error('Unhandled exception while running processor %s on sample %s: %s' % (processor_name, sample['id'], str(e)))
