...
class Host(Processor):
    ...
    def process(self, sample):
        ...
        ioc_type = "ipv4s" if metadata["ip_version"] == 4 else "ipv6s"
        self.add_ioc(ioc_type, [metadata["ip"]])

        if ip_info.is_global and metadata["ip_version"] == 4:
            ...
            if geo:
                ...
                metadata["state"] = (
                    ...
                    if geo.subdivisions.most_specific.name
                    else None
                )
                metadata["country"] = geo.country.name if geo.country.name else None
                metadata["country_code"] = (
                    geo.country.iso_code if geo.country.iso_code else None
                )
        ...
        if "geo_coordinates" in metadata.keys():
            ...
            self.extract_meta_sample(
                "location", json.dumps(metadata["geo_coordinates"]), sample["id"]
            )
