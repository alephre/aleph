    ...
    def collector(self):
        try:
            conn = poplib.POP3_SSL(
                self.options.get("pop_server"), self.options.get("pop_port")
            )
            conn.user(self.options.get("pop_user"))
            conn.pass_(self.options.get("pop_password"))
            self.logger.info(
                "Connected to POP server {0}".format(self.options.get("pop_server"))
            )
        ...
        if message_count > 1:
            ...
            # @FIXME use constant, what -1 means?
            for i in range(message_count, mailbox_size, -1):
                try:
                    ...
                    self.logger.error(
                        "Unhandled error retreiving email messages: {0}".format(
                            str(err)
                        )
                    )
            ...
            self.logger.info(
                "No new email messages to retrieve {0}".format(
