...
DATASTORES = [
    ...
    for name, options in settings.get("datastore").items()
]
...
def update_task_states(self):
    ...
    for name, datastore in DATASTORES:
        try:
            ...
            self.logger.warn(
                "Datastore update_task_states action failed: %s. Staging for retry"
                % e.get_exception_text()
            )
            self.retry(exc=e)
        except BaseException as e:
            self.logger.error(
                "Error running update_task_states on datastore %s: %s"
                % (name, e.get_exception_text())
            )
        except Exception as e:
            self.logger.error(
                "Unhandled exception while running update_task_states on datastore %s: %s"
                % (name, str(e))
            )
...
def store(self, sample_id, metadata):
    ...
    for name, datastore in DATASTORES:
        try:
            ...
            self.logger.warn(
                "Datastore store action failed: %s. Staging for retry"
                % e.get_exception_text()
            )
            self.retry(exc=e)
        except BaseException as e:
            self.logger.error(
                "Error storing data on %s datastore: %s"
                % (name, e.get_exception_text())
            )
        except Exception as e:
            self.logger.error(
                "Unhandled exception while storing data on %s datastore: %s"
                % (name, str(e))
            )
...
def track(self, sample_id, metadata):
    ...
    for name, datastore in DATASTORES:
        try:
            ...
            datastore.track(sample_id, metadata)
            self.logger.debug("Tracking data for %s stored on datastore" % sample_id)
        except DatastoreTemporaryException as e:
            self.logger.warn(
                "Datastore track action failed: %s. Staging for retry"
                % e.get_exception_text()
            )
            self.retry(exc=e)
        except BaseException as e:
            self.logger.error(
                "Error storing tracking data on %s datastore: %s"
                % (name, e.get_exception_text())
            )
        except Exception as e:
            self.logger.error(
                "Unhandled exception while storing tracking data on %s datastore: %s"
                % (name, str(e))
            )
