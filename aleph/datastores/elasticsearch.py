...
class Elasticsearch(Datastore):
            ...
            "query": {
                "bool" : {
                    "filter" : [
                        ...
                        {"script" : {"script" : {"source": "!doc['tags'].contains('processing_completed')", "lang": "painless"}}}
                    ]
                }
            },
            "stored_fields": []
    ...
    def _update(self, sample_id, document_body, index=None):
        ...
        """
        target_sample = 'aa924d570bf60a060496417291f6597593fdb100c8df05f0873fc75b781ca944'
        if sample_id == target_sample:
            self.logger.error('---- MARK II -----')
