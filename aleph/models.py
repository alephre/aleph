...
* Component - Plugin > (Processor, Analyzer)
            - Datastore
            - Collector
            - Storage
            - Classifier
...
class AlephTask(Task):
    ...
    def on_retry(self, exc, task_id, args, kwargs, einfo):
        self.logger.debug(
            "Task %s[%s] is being queued for retry: %s" % (self.name, task_id, einfo)
        )
...
class Component(ABC):
    ...
    def __init__(self, options=None, logger=None, dry=False):
        ...
        if not self.name:
            self.name = self.__class__.__name__.lower().replace(
        ...
        try:
            ...
            self.logger.error(
                "Error starting component %s: %s" % (self.__class__.__name__, str(e))
            )
    ...
    def validate_options(self):
        for option in self.required_options:
            if not self.options.has_option(option):
                raise KeyError(
                    'Required option "%s" not defined for %s handler'
                    % (option, self.name)
                )
...
class Plugin(Component):
    ...
    def add_tag(self, tag):
        if "tags" not in self.document_meta:
...
class Processor(Plugin):
    ...
    def process(self, sample):
        raise NotImplementedError(
            "Process routine not implemented on %s plugin" % self.name
        )

    def add_ioc(self, ioc_type, ioc_values):

        if "iocs" not in self.document_meta:
    ...
    def extract_meta_sample(self, meta_type, meta_data, sample_id):
        ...
        try:
            ...
            self.dispatch(
                filedata, metadata=metadata, filename=filename, parent=sample_id
            )
        except Exception as e:
...
class Datastore(Component):
    ...
    def update_task_states(self):
        raise NotImplementedError(
            "Update task states routine not implemented on %s datastore handler"
            % self.name
        )

    def retrieve(self, sample_id):
        raise NotImplementedError(
            "Retrieve routine not implemented on %s datastore handler" % self.name
        )

    def store(self, sample_id, document):
        raise NotImplementedError(
            "Store routine not implemented on %s datastore handler" % self.name
        )
    ...
    def dispatch(self, sample_id, metadata):
        ...
        sample = {"id": sample_id, "metadata": metadata}
...
class Storage(Component):
    ...
    def retrieve(self, sample_id):
        raise NotImplementedError(
            "Retrieve routine not implemented on %s storage handler" % self.name
        )

    def store(self, sample_id, data):
        raise NotImplementedError(
            "Store routine not implemented on %s storage handler" % self.name
        )
...
class Collector(Component):
    ...
    def collect(self):
        raise NotImplementedError(
            "Collection routine not implemented on %s collector" % self.name
        )
...
class Classifier(Component):
    ...
    def detect(self, sample):
        raise NotImplementedError(
            "Detect routine not implemented on %s filter" % self.name
        )
